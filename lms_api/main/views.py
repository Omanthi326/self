            ...
            for i in range(len(submissions)):
                for j in range(i + 1, len(submissions)):
                    try:
                        ...
                        # Add frontend data for interactive highlighting
                        if 'frontend_data' in similarity_result:
                            # Convert the frontend_data structure to the expected format for the UI
                            text1_segments = similarity_result['frontend_data']['text1']
                            text2_segments = similarity_result['frontend_data']['text2']
                            
                            result.update({
                                'text1': text1_segments,
                                'text2': text2_segments
                            })
...
@csrf_exempt
def submission_content(request):
    """
    Retrieve the actual text content of a submission for display in the frontend.
    
    POST parameters:
    - submission_id: ID of the submission to retrieve content for
    
    Returns:
    - JSON response with submission details and text content
    """
    if request.method != 'POST':
    ...
    try:
        ...
        if not submission_id:
            return JsonResponse({'status': 'error', 'message': 'Submission ID is required'})
        
        # Get the submission
        submission = StudentSubmission.objects.get(id=submission_id)
        
        # Get the file path
        file_path = submission.get_absolute_file_path()
...
def detailed_comparison(request):
    """
    Get detailed text comparison data between two submissions.
    
    POST parameters:
    - submission_id1: ID of the first submission
    - submission_id2: ID of the second submission
    
    Returns:
    - JSON with text segments and similarity data
    """
    if request.method != 'POST':
        return JsonResponse({'status': 'error', 'message': 'Only POST method allowed'})
    ...
    try:
        data = json.loads(request.body)
        submission_id1 = data.get('submission_id1')
        submission_id2 = data.get('submission_id2')
        
        if not submission_id1 or not submission_id2:
            return JsonResponse({'status': 'error', 'message': 'Both submission IDs are required'})
        
        # Get the submissions
        submission1 = StudentSubmission.objects.get(id=submission_id1)
        submission2 = StudentSubmission.objects.get(id=submission_id2)
        
        # Get the file paths
        file_path1 = submission1.get_absolute_file_path()
        file_path2 = submission2.get_absolute_file_path()
        
        # Extract text from files
        text1 = extract_text_from_file(file_path1)
        text2 = extract_text_from_file(file_path2)
        
        # Find similar sentences between the texts
        ...
        # Create text segments with highlighted matches
        if similar_sentences:
            # Sort sentences by their position in the documents
            doc1_sentences = sorted(similar_sentences, key=lambda x: x['text1_idx'])
            doc2_sentences = sorted(similar_sentences, key=lambda x: x['text2_idx'])
            
            # Extract text segments for document 1
            for match in doc1_sentences:
                text1_segments.append({
                    'text': match['text1_sentence'],
                    'isMatch': True,
                    'percentage': match['similarity']
                })
            
            # Extract text segments for document 2
            for match in doc2_sentences:
                text2_segments.append({
                    'text': match['text2_sentence'],
                    'isMatch': True,
                    'percentage': match['similarity']
                })
        
        # If no similar sentences found but there's a similarity score
        if not text1_segments and (submission1.similarity_score or submission2.similarity_score):
            # Create a dummy segment to show the similarity percentage
            similarity = submission1.similarity_score or submission2.similarity_score
            
            text1_segments = [{
                'text': f"This document contains approximately {similarity}% similar content with the other document.",
                'isMatch': True,
        ...
        return JsonResponse({
            ...
            'text1': text1_segments,
            'text2': text2_segments,
            'similarity_score': submission1.similarity_score or submission2.similarity_score or 0
        })
