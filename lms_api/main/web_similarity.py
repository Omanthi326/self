...
def generate_report_with_highlighting(analysis_results, output_path):
    """
    ...
    # NEW: Enhanced highlighting styles for different similarity levels
    high_plagiarism_style = ParagraphStyle(
        "HighPlagiarismStyle",
        parent=styles["Normal"],
        fontSize=10,
        leading=12,
    ...
    medium_plagiarism_style = ParagraphStyle(
        "MediumPlagiarismStyle",
        parent=styles["Normal"],
        fontSize=10,
        leading=12,
        spaceAfter=4,
    ...
    low_plagiarism_style = ParagraphStyle(
        "LowPlagiarismStyle",
        parent=styles["Normal"],
        fontSize=10,
        leading=12,
    ...
    # NEW: Add a legend for highlighting
    elements.append(Paragraph("Color Legend for Highlighted Text:", normal_style))
    elements.append(Paragraph("<font color='red' bgcolor='#ffcccc'>■</font> High Similarity (75-100%): Potential verbatim copying", normal_style))
    elements.append(Paragraph("<font color='orange' bgcolor='#ffeecc'>■</font> Medium Similarity (50-74%): Significant paraphrasing", normal_style))
    elements.append(Paragraph("<font color='green' bgcolor='#eeffcc'>■</font> Low Similarity (30-49%): Minor similarities", normal_style))
    elements.append(Spacer(1, 12))
    ...
    if detailed_matches:
        for i, match in enumerate(detailed_matches):
            ...
            # Determine highlight style based on similarity percentage
            highlight_style = low_plagiarism_style
            if similarity >= 75:
                highlight_style = high_plagiarism_style
            elif similarity >= 50:
                highlight_style = medium_plagiarism_style
            
            # Match content with enhanced highlighting
            assignment_text = match.get('assignment_text', 'N/A')
            source_text = match.get('source_text', 'N/A')
            
            elements.append(Paragraph("<b>Assignment:</b>", match_style))
            elements.append(Paragraph(assignment_text, highlight_style))
            
            elements.append(Paragraph("<b>Source:</b> " + match.get('source_url', 'N/A'), match_style))
            elements.append(Paragraph(source_text, normal_style))
    ...
    elements.append(Paragraph("<i>Sections highlighted indicate potential plagiarism, with color indicating similarity level.</i>", styles["Italic"]))
    elements.append(Spacer(1, 10))
    ...
    for match in detailed_matches:
        similarity = match.get("similarity", 0)
        if similarity > 30:  # Only highlight sections with significant similarity
            plagiarized_text = match.get("assignment_text", "")
            if plagiarized_text:
                plagiarized_sections[plagiarized_text] = {
    ...
    for paragraph in paragraphs:
        ...
        plagiarized = False
        for plagiarized_text, info in plagiarized_sections.items():
            if plagiarized_text in paragraph:
                ...
                highlighted_paragraph = paragraph.replace(
                    plagiarized_text, 
                    f'<font color="{text_color}" bgcolor="{highlight_color}">{plagiarized_text}</font>'
                )
                elements.append(Paragraph(highlighted_paragraph, normal_style))
                elements.append(Paragraph(f"<i><font size='8'>Source: {source_url} - Similarity: {similarity}%</font></i>", styles["Italic"]))
                plagiarized = True
                break
